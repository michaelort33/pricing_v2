...
X = [(a, b) for a, b in permutations(range(t), 2) if a < b and b-a < max_stay]
A = np.apply_along_axis(lambda x: collides(
    X[x[0]], X[x[1]]), 0, np.meshgrid(range(len(X)), range(len(X))))

# %% Simulate
n = 1000  # Number of times to simulate

Q = [q(x) for x in X]  # evaluate nightly prices
# Use prices to find isolated probabilities
P = [p(q, x) for q, x in zip(Q, X)]
req = np.random.rand(n, len(X)) < P  # n trials of reservation requests
order = np.stack([np.random.permutation(len(X))
                  for i in range(n)])  # n trials of precedence
...
Elos = np.mean([np.mean(res_len(np.array(X)[x]))
